...
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query"; // useQueryClient and useMutation added
...
import { FaArrowLeft, FaTrophy } from "react-icons/fa"; // FaTrophy added
...
const ContestSubmissions = () => {
  ...
  const handleDeclareWinner = (submission) => {
    const confirmed = window.confirm(
      `Are you sure you want to declare ${submission.name} as the winner?`
    );
  ...
  const winnerDeclared = !!contestDetails?.winner; // উইনার ডিক্লেয়ার হয়েছে কি না
      ...
      {submissions.length === 0 ? (
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          <p className="text-xl text-gray-600 font-medium">
            No submissions found yet for this contest. 😟
          </p>
        </div>
      ) : (
              ...
              {submissions.map((sub) => {
                ...
                return (
                  <tr key={sub._id} className={isWinner ? "bg-green-50" : ""}>
                    <td className="px-5 py-5 whitespace-nowrap text-sm font-medium text-gray-900">
                      {sub.name}
                    </td>

                    <td className="px-5 py-5 whitespace-nowrap text-sm text-gray-600">
                      {sub.email}
                    </td>

                    <td className="px-5 py-5 text-sm text-gray-500 break-words max-w-sm">
                      {sub.task}
                    </td>
                    ...
                    <td className="px-5 py-5 whitespace-nowrap text-sm">
                      {isWinner ? (
                        ...
                        <button
                          ...
                          disabled={
                            isDisabled || declareWinnerMutation.isPending
                          }
